...
module Bud
  ...
  private
  def build_defs
    @build_defs = self.class.instance_methods.select {|m| m =~ /^__build__.+$/}.map {|m| m.to_s}
    # @build_defs.each do |m|
    #   puts m
    # end
  end
  ...
  private
  def sugar_defs
    @sugar_defs = self.class.instance_methods.select {|m| m =~ /^__sugar__.+$/}.map {|m| m.to_s}
    # @sugar_defs.each do |m|
    #   puts m
    # end
  end

  private
  def absorb_imports(local_name, mod_inst)
  ...
  def resolve_imports
    ...
    import_tbl.each_pair do |local_name, mod_name|
      ...
      absorb_imports(local_name, mod_inst)
    end

    build_tbl = build_defs
    build_tbl.each do |meth_name|
      puts "Parsing builds: #{meth_name}"
      spec = self.class.__build_blocks__[meth_name]
      ...
      if mod_inst.nil?
        ...
        builder = spec[:builder].new(self, local_name)
        Docile.dsl_eval(builder, &spec[:block])
        Docile.dsl_eval builder do
          opts :toplevel => toplevel, :qualified_name => qlocal_name
        end

        mod_inst = builder.build
        instance_variable_set("@#{local_name}", mod_inst)
      end
      absorb_imports(local_name, mod_inst)
    end

    sugar_tbl = sugar_defs
    sugar_tbl.each do |meth_name|
      puts "Parsing sugar blocks: #{meth_name}"
      spec = self.class.__sugar_blocks__[meth_name]
      local_name = spec[:local_name]
  ...
  def do_bootstrap
    ...
    build_defs.each do |meth_name|
      o = self.class.__build_blocks__[meth_name]
      wrapper = import_instance o[:local_name]
      wrapper.do_bootstrap
    end
